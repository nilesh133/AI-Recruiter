"use client";
import { Card, CardHeader, CardBody, CardFooter, Accordion, AccordionItem, Spinner } from "@heroui/react";
...
export default function InterviewReport({ params }: InterviewPageProps) {
...
//     "Describe the useCallback and useMemo hooks.",
...
console.log("attendees", attendees);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-10 min-w-[70%]">
      <h1 className="text-3xl font-semibold text-white text-center">Interview Details</h1>
      {/* Interview Info Card */}
      <Card className="bg-gradient-to-br from-zinc-900 to-zinc-800 text-white shadow-xl min-w-[70%]">
        <CardHeader className="text-2xl font-semibold border-b border-zinc-700 pb-2">{interview?.profile}</CardHeader>
              ...
              <AccordionItem title="Interview Questions">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {questions?.map((question, index) => (
                    <div key={index} className="bg-zinc-700 p-4 rounded-md text-gray-200">
                      <span className="font-semibold">Q{index + 1}:</span> {question.question}
                      <br/>
                      <span className="font-semibold text-blue-500"></span>Type: {question.type}
                    </div>
                  ))}
                </div>
              </AccordionItem>
      ...
      {/* Attendees Card */}
      <Card className="bg-gradient-to-br from-zinc-900 to-zinc-800 text-white shadow-xl min-w-[70%]">
        <CardHeader className="text-2xl font-semibold border-b border-zinc-700 pb-2">Attendees</CardHeader>
        <CardBody className="space-y-6">
            ...
            <div key={index} className="flex justify-between items-center border-b border-zinc-700 pb-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">Interviewed on {new Date(user?.createdAt)?.toLocaleString() || '--'}</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-xl font-bold text-blue-400">{JSON.parse(user?.data)?.overall_score}/10</span>
                <Button onPress={() => {
                  router.push(`/interview-report/${interviewId}/userreport/${user.id}`);
                }} className="hover:bg-blue-600 text-white duration-300 ease-in transition-all" color="primary" variant="bordered" size="md">View Report</Button>
              </div>
            </div>
          ))}
        </CardBody>
